...
class PreferenceManager {
  ...
  clearCache = async () => {
    ...
    await Promise.all(
      Object.values(STORAGE_KEYS).map((key) => cache.removeItem(key)),
    );
  };
  ...
  private async loadCustomFromCache(): Promise<CustomPreferencesRecord | null> {
    return this.getCache().getItem<CustomPreferencesRecord>(
      STORAGE_KEYS.CUSTOM,
    );
  }
  ...
  private async loadFromCache(): Promise<null | Preferences> {
    return this.getCache().getItem<Preferences>(STORAGE_KEYS.MAIN);
  }
  ...
  private async saveToCache() {
    ...
    try {
      await cache.setItem(STORAGE_KEYS.MAIN, this.state);
      await cache.setItem(STORAGE_KEYS.LOCALE, this.state.app.locale);
      await cache.setItem(STORAGE_KEYS.THEME, this.state.theme.mode);

      if (this.customPreferencesExtension) {
        await cache.setItem(STORAGE_KEYS.CUSTOM, {
          ...this.customState,
        });
        return;
      }

      await cache.removeItem(STORAGE_KEYS.CUSTOM);
    } catch (error) {
