...
云程快速开发平台是一套面向企业应用的开发底座与可组合业务产品平台。它提供统一的前后端工程、系统管理能力和技术基础设施，适合作为内部管理平台、业务运营后台及其他业务产品的共享基础。
...
- 为后续业务产品提供一致的代码组织、运行装配和扩展边界。

项目采用单租户模型。需要在企业之间实现完全隔离时，可以通过独立应用、数据库和 Redis 进行部署隔离。

## 2. 总体架构

![云程快速开发平台架构图](images/architecture.svg)
...
批量 Excel 导入使用应用级 `ExcelImportDialog` 统一模板下载、文件选择、可配置业务提示、提交反馈和结构化错误展示；工作簿列、业务权限、后端校验和事务仍由各业务模块负责，公共组件不建设万能导入协议。后端用户和组织导入复用稳定的导入结果与行级错误结构，不抽取依赖具体工作簿字段的万能处理器。
...
用户模板和导出统一采用带格式版本的交换工作簿，系统导出的文件可以直接在另一套部署中导入。文件使用组织编码、角色编码表达关系，不携带数据库主键；目标系统重新生成用户主键，并按自身编码数据和公共默认密码完成创建。空白模板的参考页签列出当前操作者可分配的全部组织和角色，用户导出文件的参考页签只列出本批用户实际引用的组织和角色。导入与导出统一限制为最多 10000 个用户、文件不超过 20 MB；导入只新增用户，整批校验通过后在一个事务中写入，任一错误均不产生部分数据。模板下载和导入使用用户新增权限，包含未脱敏联系方式的导出使用独立导出权限。
...
组织管理在系统领域内维护组织、部门、小组三种节点，支持多个顶级组织、不分页异步树加载、按名称与编码组合查询，以及节点新增、编辑、移动、删除和 Excel 导入导出。节点不设置启停状态，使用父节点关系和物化路径同时表达结构；只有名称变化才重建后代节点的完整路径，编码、排序号或说明等普通编辑只更新当前节点。界面按变更范围局部刷新，名称变更和移动等结构性操作刷新树时恢复仍有效的展开节点。用户归属组织在用户管理模块中维护，组织管理页面不承担人员查看。

组织模板和导出使用带格式版本的交换工作簿，数据字段只保存组织编码、名称、类型、上级组织编码、排序号和说明，不携带数据库主键、物化路径、层级或审计字段。系统导出排除固定主键的内置默认组织，其他节点仍可使用 `default` 作为上级编码；导入目标将该编码解析为自身的内置默认组织，因此完整导出文件可以导入另一套新部署，而不会覆盖目标系统的默认组织。导入只新增节点，已有普通组织编码一律作为冲突返回，不隐式覆盖、移动、删除或跳过。

组织导入不依赖 Excel 行顺序。后端先读取整份文件，在内存中校验编码和同级名称唯一性、上级存在性、循环关系、节点类型嵌套以及与目标现有路径组合后的最大深度，再按父节点优先顺序在一个事务中写入；任一行失败均不产生部分数据。导入和导出统一限制为最多 10000 个组织、文件不超过 20 MB。模板下载和导入复用组织新增权限，导出使用独立的组织导出权限。导入与导出都记录操作日志。
...
功能规模较小时可以放入现有领域模块；形成独立业务边界后，应增加与 `yuncheng-system` 平行的 Maven 模块。业务产品可以使用 System 公开契约和所需 Framework 能力，System 与 Framework 不反向依赖具体业务产品。

## 17. 当前版本边界

`v1.0.0` 是平台基础底座的首个正式版本，采用以下边界：
