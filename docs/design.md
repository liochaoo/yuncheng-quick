...
- `AsyncOrgTree`：当前用于用户管理左侧组织筛选，按父节点异步加载组织树，支持名称、编码和完整路径搜索；
...
组织树和组织搜索均不分页。树形浏览按父节点异步加载，关键字搜索返回完整匹配集合，前端不进行切片假分页，也不展示组织分页控件。表格查询入口对执行期间的新查询进行末次合并，并在当前查询完成后补发；依赖组织筛选等外部条件的列表还需校验请求快照，避免旧条件的数据被解释为当前结果。
...
每个用户至少直接归属于一个组织、部门或小组节点，可以拥有多条组织归属，但有且只有一条主归属。同一用户不能重复绑定同一个直接归属节点，同一组织树中的祖先和后代节点允许同时绑定。

用户和组织通过 `system_user_org` 维护关系。关系只保存直接归属节点和主归属标记，完整路径、所在及顶级组织、部门和小组根据 `system_org` 的当前路径动态推导。组织移动不批量修改用户关系。

用户管理页面使用左侧异步组织树和右侧用户列表。组织筛选支持直属或包含下级、全部归属或仅主归属或仅其他归属；分页查询按用户去重。用户列表只集中展示主归属和其他归属数量，用户详情展示全部组织上下文。

组织归属回答“用户属于哪里”，不自动产生数据访问权限。后续数据权限需要独立定义基于主归属、全部归属或指定组织节点的授权策略。
...
5. Redis 创建在线 Session 和 Refresh Token 索引；
6. Access Token 返回给前端，Refresh Token 写入 HttpOnly Cookie；
7. 前端加载用户信息、权限码和动态菜单。
...
组织管理在系统领域内维护组织、部门、小组三种节点，支持多个顶级组织、不分页异步树加载、不分页名称或编码或完整路径搜索，以及节点新增、编辑、移动和删除。节点不设置启停状态，使用父节点关系和物化路径同时表达结构；只有名称变化才重建后代节点的完整路径，编码、排序号或说明等普通编辑只更新当前节点。用户组织归属在用户管理模块中维护，组织管理页面不承担人员查看。

平台使用固定主键初始化“默认组织”。初始管理员、示例用户、批量演示用户和公开注册用户默认以该节点作为主归属。用户管理新增和编辑必须显式提交至少一条组织归属及主归属。
...
数据字典由字典和字典选项组成，用于维护状态、类型等小规模参考数据。公共查询接口只返回可用于业务选择的选项，管理接口用于维护字典内容。
...
`v0.1.0-alpha.1` 是早期预览版本，当前采用以下边界：
