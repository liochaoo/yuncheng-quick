...
- `AsyncOrgTree`：按父节点异步加载组织树，支持名称、编码和完整路径搜索；
- `OrgSelectionDialog`：通用组织选择弹框，支持单选、多选、节点类型限制和排除指定子树；
- `OrgSelect`：单组织字段包装组件，通过组织选择弹框完成选择和已选值恢复。

组织管理和用户管理可以使用不同权限的接口，但共享同一套组件和组织摘要契约。组件通过明确的权限场景选择对应接口，不绕过后端权限校验。
...
组织管理在系统领域内维护组织、部门、小组三种节点，支持多个顶级组织、不分页异步树加载、名称或编码或完整路径搜索，以及节点新增、编辑、移动和删除。节点不设置启停状态，使用父节点关系和物化路径同时表达结构；用户组织归属在用户管理模块中维护，组织管理页面不承担人员查看。
