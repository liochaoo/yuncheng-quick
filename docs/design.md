...
Flyway 在配置中显式设置为默认开启，应用首次连接空数据库时会自动执行建表和系统基础数据迁移，也可以通过配置关闭。

仓库根目录的 `database/mysql/<版本>/` 同时提供按发布版本整理的全量脚本和版本间增量脚本，用于手动初始化或升级数据库。具体使用方式参阅 [`database/README.md`](../database/README.md)。
...
- 前端接口按业务领域放入 `api` 目录；
- 菜单、路由和权限码与后端授权保持对应；
- 数据库变化随发布版本提供 Flyway 迁移，并同步提供独立的全量和增量脚本。
